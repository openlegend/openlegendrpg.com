...
(function(process) {
  ...
  FsEventsHandler.prototype._watchWithFsEvents = function(watchPath, realPath, processPath) {
    ...
    var watcher = setFSEventsListener(watchPath, realPath, watchCallback, this.emit.bind(this, 'raw'));
    this._emitReady();
    return this._watchers.push(watcher);
  };
  FsEventsHandler.prototype._handleSymlinkForFsEvents = function(linkPath, pathTransform) {
    this._readyCount++;
    fs.realpath(linkPath, function(error, linkTarget) {
      ...
      this._addToFsEvents(linkTarget, function(path) {
        var ds = '.' + sysPath.sep;
        return pathTransform(linkTarget && linkTarget !== ds ? path.replace(linkTarget, linkPath) : path === ds ? linkPath : sysPath.join(linkPath, path));
      });
    }.bind(this));
  };
  FsEventsHandler.prototype._addToFsEvents = function(file, pathTransform, forceScan) {
    var processPath = typeof pathTransform === 'function' ? pathTransform : function(val) {
      return val;
    };
