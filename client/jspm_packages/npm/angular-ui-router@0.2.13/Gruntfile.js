...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    uglify: {
      ...
      build: {files: {'<%= builddir %>/<%= pkg.name %>.min.js': ['<banner:meta.banner>', '<%= concat.build.dest %>']}}
    },
    release: {
    ...
    karma: {
      ...
      unit: {browsers: [grunt.option('browser') || 'PhantomJS']},
      debug: {
        singleRun: false,
        background: false,
        browsers: [grunt.option('browser') || 'Chrome']
      },
      past: {configFile: 'config/karma-1.0.8.js'},
      unstable: {configFile: 'config/karma-1.1.5.js'},
      future: {configFile: 'config/karma-1.3.0.js'},
      background: {
        background: true,
        browsers: [grunt.option('browser') || 'PhantomJS']
      },
  ...
  grunt.registerTask('widedocs', 'Convert to bootstrap container-fluid', function() {
    promising(this, system('sed -i.bak -e \'s/class="row"/class="row-fluid"/\' -e \'s/role="main" class="container"/role="main" class="container-fluid"/\' site/index.html'));
  });
  ...
  grunt.registerTask('perform-release', function() {
    ...
    promising(this, system('git add \'' + releasedir + '\'').then(function() {
