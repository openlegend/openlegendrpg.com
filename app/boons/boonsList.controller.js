...
System.register(["angular", "lodash", "./boons.json!"], function (_export, _context) {
  ...
  var angular, boons, BoonsListCtrl;
  ...
  return {
    ...
    execute: function () {
      ...
      function () {
        ...
        function BoonsListCtrl($scope, $location, $mdMedia, Config, Link) {
          var _this = this;

          _classCallCheck(this, BoonsListCtrl);
          ...
          this.Config = Config;
          this.Link = Link;
          this.items = boons || []; // actively generate a uniq array of all attributes used in the json resource

          this.filterAttributes = _.uniq(_.flatten(_.map(boons, 'attribute'))).map(function (attr) {
            return {
              name: attr
            };
          });
          ...
          this.$scope.$watch('boonsCtrl.powerLevelValue', function (newVal, oldVal) {
          ...
          this.$scope.$watch('boonsCtrl.selectedAttributes', function (newVal, oldVal) {
            // clear all params
            ...
            return function (thisPower) {
              ...
              if (_.compact(_.map(_this4.selectedAttributes, 'power')).length > 0) {
                for (var i = 0; i <= item.power.length; i++) {
                  ...
                  var intersectAttributes = _.intersection(item.attribute, _.map(_this4.selectedAttributes, 'name'));
        ...
        return BoonsListCtrl;
      }();

      _export("default", BoonsListCtrl);
    }
  };
});
//# sourceMappingURL=boonsList.controller.js.map
